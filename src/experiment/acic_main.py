...
def train_and_predict_dragons(t, y_unscaled, x, targeted_regularization=True, output_dir='',
    ...
    test_outputs = []
    runs = 5
    # Train and run network multiple times so later results can be averaged
    for i in range(runs):
        ...
        # Predict new dragonnet output on test data and train data
...
def run_acic(data_base_dir='../dat/', output_dir='../result/acic/',
             knob_loss=dragonnet_loss_binarycross,
             ratio=1., dragon='', folder='a'):
    """
    Method for laoding the data and running the different networks
    Args:
        data_base_dir: Database folder
        output_dir: Folder for writing results too
        knob_loss: What loss to use
        ratio:
        dragon:
        folder:
